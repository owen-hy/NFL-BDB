...
tracking_df = pd.read_csv('tracking_week_3.csv')
...
def animate_play(games, tracking_df, play_df, players, gameId, playId):
    ...
    """
    # filters to that game and play for each df
    ...
    print(f"Unique clubs in tracking data for gameId {gameId} and playId {playId}: {selected_tracking_df['club'].unique()}")

    # gets list of all unique frames
    ...
    print(f"Team combinations identified: {team_combos}")
    ...
    layout = go.Layout(
        ...
        # can I bold motion type? or make it bigger font
        title=f"""<span style="font-weight:bold; font-size:24px;">Motion Type: Return</span><br>GameId: {gameId}, PlayId: {playId}""" + "<br>" * 19 + f"{playDescription}",
